        ...
        for file_item in a.body:
                ...
                for class_item in file_item.body:
                    if type(class_item) is ast.FunctionDef:
                        ...
                        if not re.match(private_function, class_item.name):
                            func_name, args = extract_python_ast_args(
                                class_item,
                                True)
                            functions.append(
                                ((file_item.name + "." + func_name), args))
            if type(file_item) is ast.FunctionDef:
                if not re.match(private_function, file_item.name):
                    ...
                    functions.append(
                        ((source_file.split("\\")[0] + "." + func_name), args))
    return functions
...
def extract_python_ast_args(func_def, class_method):
    ...
    return func_def.name, (
        (len(required) - 1 if class_method else len(required)),
        (sys.maxsize if hasattr(func_def.args, "kwarg") else len(optionals)))
