    ...
    with open(truth_file, "r", encoding="utf-8") as temp:
        ...
        with open(test_file, "r", encoding="utf-8") as temp:
            ...
            with open(os.path.normpath("comparison/" + truth_file.split("\\")[1]), "w", encoding="utf-8", newline="") as results:
                ...
                result_writer.writerow(["Example", "Linked function", "Truth function", "Source", "Correct"])
                truth_headers = ["example", "page", "source", "class", "function"]
                ...
                for row1 in truth_list:
                    ...
                    for row2 in test_list:
                        ...
                        if truth_row["example"] == test_row["example"]:
                            if truth_row["function"] == test_row["linked"] and truth_row["source"] == test_row["source"] and test_row["matched"] == "True":
                                result_writer.writerow([truth_row["example"], test_row["linked"], truth_row["function"], truth_row["source"], 1])
                                correct += 1
                            else:
...
if __name__ == '__main__':
    with open(os.path.normpath("comparison/evaluation.csv"), "w", encoding="utf-8", newline="") as out_file:
        writer = csv.writer(out_file, quoting=csv.QUOTE_MINIMAL)
        writer.writerow(["Repo", "Correct", "Total test", "Precision", "Correct", "Total truth", "Recall"])
        for file1 in os.listdir("truth"):
            ...
            for file2 in os.listdir("results"):
                ...
                if file1 == file2:
                    writer.writerow([file1.split(".")[0], *evaluate_links(truth_file, test_file)])
                    break
